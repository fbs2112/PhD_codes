...
monteCarloLoops = length(trueLabels);

detection_res = zeros(monteCarloLoops, length(thresholdVector), length(window_median_length_vector));
for loopIndex = 1:monteCarloLoops
    loopIndex
    load(['.' filesep 'data' filesep 'dataParkes_' num2str(trueLabels(loopIndex)) '.mat']);   
    [W, ~, e, PxxAux, f, t] = nmf_eval_detection(parkesSignal, params);
...
save(['.' filesep 'data' filesep 'results_det_3.mat'], 'detection_res', '-v7.3');
