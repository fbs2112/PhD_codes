...
%--------------------------------------------
[W, H, reconstructError, PxxAux, f, t] = nmf_eval(mixtureSignal, params, true);
dataCellName = {'', 'TK'};
...
for JNRIndex = 1:length(params.JNRVector)
    for dataIndex = 1:size(PxxAux, 1)
        ...
        if save_fig
            formatFig(gcf, [dataPath 'spectrogram_detect_chirp_' dataCellName{dataIndex}], 'en', figProp);
        end
        ...
        for i = 1:numberOfSources
            ...
            if save_fig
                formatFig(gcf, [dataPath 'basis_detect_chirp_' num2str(i) '_' dataCellName{dataIndex}], 'en', figProp);
            end
            figure;
            ...
            xlim([min(t) max(t)]*1e6);
            if save_fig
                formatFig(gcf, [dataPath 'activation_detect_chirp_' num2str(i) '_' dataCellName{dataIndex}], 'en', figProp);
            end
            ...
            if save_fig
                formatFig(gcf, [dataPath 'activation_mean_detect_chirp_' num2str(i) '_' dataCellName{dataIndex}], 'en', figProp);
            end
        ...
        xlim([1 idx])
        if save_fig
            formatFig(gcf, [dataPath 'loss_error_detect_chirp' '_' dataCellName{dataIndex}], 'en', figProp);
        end
        ...
        for k = 1:numberOfSources
            ...
            if save_fig
                formatFig(gcf, [dataPath 'output_spectrogram_detect_chirp_' num2str(k) '_' dataCellName{dataIndex}], 'en', figProp);
            end
        end
    end
end
