...
function [W, H, reconstructError] = nmf_vanilla_orthogonal_H(X, params, varargin)
...
diagMtx = diag([ones(numberOfComponentsPerSource(1),1);zeros(numberOfComponentsPerSource(2), 1)]);
...
elseif betaDivergenceAux == 1
    reconstructError0 = KL_divergence(X, W0, H0, 'ort_H', params.mu_orth, numberOfComponentsPerSource);
end
...
%             numH3 = W.'*(X./(W*H0 + eps(X))) + params.mu_orth(2)*H0;
%             denH3 = W.'*onesMtx + eps(numH3) + params.mu(2) + params.mu_orth(2)*onesMtx2*H0;
            
            numH = W.'*(X./(W*H0 + eps(X))) + diagMtx*params.mu_orth(2)*H0;
            denH = W.'*onesMtx + eps(numH) + params.mu(2) + params.mu_orth(2)*diagMtx*onesMtx2*diagMtx*H0;
           
            
%             numH2 = W.'*(X./(W*H0 + eps(X)));
%             denH2 = W.'*ones(size(W, 1), size(H0, 2)) + eps(numH) + params.mu(2);
%             numH1 = W(:,1:numberOfComponentsPerSource(1)).'*(X./(W(:,1:numberOfComponentsPerSource(1))*H0(1:numberOfComponentsPerSource(1),:) + eps(X))) + ...
%                 params.mu_orth(2)*H0(1:numberOfComponentsPerSource(1),:);
%             denH1 = W(:,1:numberOfComponentsPerSource(1)).'*onesMtx + eps(numH1) + params.mu(2) + ...
%                 params.mu_orth(2)*onesMtx2(1:numberOfComponentsPerSource(1),1:numberOfComponentsPerSource(1))*H0(1:numberOfComponentsPerSource(1),:);
%             
%             numH2 = W(:,numberOfComponentsPerSource(1)+1:end).'*(X./(W(:,numberOfComponentsPerSource(1)+1:end)*H0(numberOfComponentsPerSource(1)+1:end,:) + eps(X)));
%             denH2 = W(:,numberOfComponentsPerSource(1)+1:end).'*onesMtx + eps(numH2) + params.mu(2); 
...
%             denH = [denH1;denH2];
        ...
        reconstructError(i) = KL_divergence(X, W, H, 'ort_H', params.mu_orth, numberOfComponentsPerSource);
    end
