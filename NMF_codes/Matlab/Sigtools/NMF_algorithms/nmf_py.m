function [wbest,hbest,normbest] = nmf_py(a,k,varargin)
...
% ---- Nested functions ----
    ...
    function cellout = loopBody(iter,S)
        ...
        [whtry{3},whtry{4},whtry{1}] = ...
            nnmfKL(a,whtry{3},whtry{4},maxiter,tolfun,tolx,...
...
function [w,h,dnorm] = nnmfKL(a,w0,h0,maxiter,tolfun,tolx,...
                                   dispnum,repnum,usePool)
% Single non-negative matrix factorization
nm = numel(a);
sqrteps = sqrt(eps);
beta = 'kullback-leibler';
square_root = true(1);
...
for j=1:maxiter
    ...
    numW = (a./(w0*h0 + eps(a)))*h0.';
    denW = (ones(size(h0, 2))*h0.') + eps(numW);
...
%     d = a - w*h;
%     dnorm = sqrt(sum(sum(d.^2))/nm);
    dnorm = beta_divergence(a, w, h, beta, square_root);
