...
paramsVector = [8 16 32 64 128];

detection_res = zeros(monteCarloLoops, length(bandwidthVector), length(periodVector), ...
    length(params.JNRVector), length(thresholdVector), length(window_median_length_vector),length(paramsVector));
...
for loopIndex = 1:monteCarloLoops
    ...
    for paramsIndex = 1:length(paramsVector)
        
        params.nfft = paramsVector(paramsIndex);
        params.nperseg = paramsVector(paramsIndex);
                ...
                for JNRIndex = 1:length(params.JNRVector)
                    ...
                    for thresholdIndex = 1:length(thresholdVector)
                        for window_median_length_index = 1:length(window_median_length_vector)
                            detection_res(loopIndex, bandwidthIndex, periodIndex, JNRIndex, thresholdIndex, window_median_length_index, paramsIndex) = ...
...
save(['.' filesep 'data' filesep 'results_det_14.mat'], 'detection_res', '-v7.3');
