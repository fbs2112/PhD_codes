...
params.JNRVector = 10:5:50;
params.fs = paramsSignal.Freqsamp;
params.nfft = 256;
params.nperseg = 256;
params.overlap = params.nperseg - 1;
params.hop_size = params.nperseg - params.overlap;
params.window = kaiser(params.nperseg, 10);
...
mixtureSignal = zeros(totalSamples, length(params.JNRVector), 1, 1, monteCarloLoops);
...
%     interferenceSignalPower = pow_eval(interferenceSignal);
    ...
    for JNRIndex = 1:length(params.JNRVector)
        ...
        mixtureSignal(:,JNRIndex,1,1,loopIndex) = filter(b, a, mixtureSignalAux);
%         mixtureSignal(:,JNRIndex,1,1,loopIndex) = mixtureSignalAux;
    end
    
    [WTest, HTest, errorTest, PxxTest, f, t] = nmf_eval_v2(mixtureSignal(:,:,1,1,loopIndex), params);
...
save(['.' filesep 'data' filesep 'nmf_testing_08.mat'], 'xHat', 'mixtureSignal', 'JNRVector');
