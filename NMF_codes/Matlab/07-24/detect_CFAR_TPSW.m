                    ...
                    for NIndex = 1:length(NVector)
                        for MIndex = 1:length(MVector)
                            for alphaIndex = 1:length(alphaVector)
                                ...
                                output = tpsw_filt(output, MVector(MIndex), NVector(NIndex), alphaVector(alphaIndex));
                                ...
                                for numberOfTrainingCellsIndex = 1:length(numberOfTrainingCellsVector)
                                    ...
                                    for numberOfGuardCellsIndex = 1:length(numberOfGuardCellsVector)
                                        ...
                                        for window_median_length_index = 1:length(window_median_length_vector)
                                            [detection_res(loopIndex, JNRIndex, thresholdIndex, window_length_index, NIndex, MIndex, alphaIndex,...
...
save(['..' filesep '.' filesep 'data' filesep '07-24' filesep 'results07.mat'], 'detection_res', '-v7.3');
