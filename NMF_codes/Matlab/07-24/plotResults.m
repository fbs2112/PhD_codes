...
load_aux = matfile('results04.mat');
    ...
    for thresholdIndex = 1:length(thresholdVector)
        for stdIndex = 1:length(stdVector)
            for window_median_length_index = 1:length(window_median_length_vector)
                ...
                fn = zeros(monteCarloLoops, 1);
                tn = zeros(monteCarloLoops, 1);
                aux = squeeze(load_aux.detection_res(:, JNRIndex, thresholdIndex, stdIndex, window_median_length_index, :));
                for loopIndex = 1:monteCarloLoops
                    
                    fpRegion1 =  aux(loopIndex, 1:onset);
                    tpRegion = aux(loopIndex, onset  + round(window_length*2):offset - round(window_length*2));
                    fpRegion2 = aux(loopIndex, offset + round(window_length*2):end);
...
for i = 1:length(JNRVector)
    ...
    disp(['Threshold: ' num2str(thresholdVector(j))])
    disp('------------------------------------')
    disp(['Std: ' num2str(thresholdVector(k))])
    disp('------------------------------------')
