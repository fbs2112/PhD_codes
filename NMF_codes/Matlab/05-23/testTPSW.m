...
for JNRIndex = 1:length(params.JNRVector)
    for dataIndex = 1:size(PxxAux, 1)
        ...
        xlabel('Time [$\mu$s]');
        ax = gca;
        set(ax, 'CLim', [-5 20], 'colormap', jet);
        c = colorbar;
        c.Label.String = '[dB]';
        c.TickLabelInterpreter = 'latex';
        
        if save_fig
            formatFig(gcf, [dataPath 'spectrogram_detect_2chirps_separated_' dataCellName{dataIndex} '_' num2str(params.JNRVector(JNRIndex))], 'en', figProp);
        end
        
        for timeIdx = 1:size(PxxAux{dataIndex, JNRIndex}, 1)
            freqComponent = filter(window_coeffs, 1, abs(PxxAux{dataIndex, JNRIndex}(timeIdx,:)).^2);
            Pxx_TPSW(timeIdx,:) = abs(PxxAux{dataIndex, JNRIndex}(timeIdx,:)).^2;
            Pxx_TPSW(timeIdx, abs(PxxAux{dataIndex, JNRIndex}(timeIdx,:)).^2 > alpha*freqComponent) =...
                freqComponent(abs(PxxAux{dataIndex, JNRIndex}(timeIdx,:)).^2 > alpha*freqComponent);
            Pxx_TPSW(timeIdx,:) = filter(window_coeffs, 1, Pxx_TPSW(timeIdx,:));
           
        end
        
        Pxx_filt = abs(PxxAux{dataIndex, JNRIndex}).^2 - Pxx_TPSW;
        Pxx_filt(Pxx_filt < 0) = eps;
        
        figure;
        surf(t*1e6, f/1e6, 10*log10(Pxx_filt), 'EdgeColor', 'none');
        axis xy;
        view(0, 90);
        xlim([min(t) max(t)]*1e6);
        ylim([min(f) max(f)]/1e6);
        ...
        figure;
        surf(t*1e6, f/1e6, 10*log10(Pxx_TPSW), 'EdgeColor', 'none');
        axis xy;
        view(0, 90);
        xlim([min(t) max(t)]*1e6);
        ylim([min(f) max(f)]/1e6);
        ...
        for i = 1:numberOfSources
            ...
            if save_fig
                formatFig(gcf, [dataPath 'basis_detect_2chirps_separated_' num2str(i) '_' dataCellName{dataIndex} '_' num2str(params.JNRVector(JNRIndex))], 'en', figProp);
            end
            figure;
            ...
            xlim([min(t) max(t)]*1e6);
            if save_fig
                formatFig(gcf, [dataPath 'activation_detect_2chirps_separated_' num2str(i) '_' dataCellName{dataIndex} '_' num2str(params.JNRVector(JNRIndex))], 'en', figProp);
            end
            ...
            if save_fig
                formatFig(gcf, [dataPath 'activation_mean_detect_2chirps_separated_' num2str(i) '_' dataCellName{dataIndex} '_' num2str(params.JNRVector(JNRIndex))], 'en', figProp);
            end
        ...
        xlim([1 idx])
        if save_fig
            formatFig(gcf, [dataPath 'loss_error_detect_2chirps_separated' '_' dataCellName{dataIndex} '_' num2str(params.JNRVector(JNRIndex))], 'en', figProp);
        end
        ...
        for k = 1:numberOfSources
            ...
            if save_fig
                formatFig(gcf, [dataPath 'output_spectrogram_detect_2chirps_separated_' num2str(k) '_' dataCellName{dataIndex} '_' num2str(params.JNRVector(JNRIndex))], 'en', figProp);
            end
        end
    end
end
