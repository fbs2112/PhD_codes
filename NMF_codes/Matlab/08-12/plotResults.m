...
%Results using GPS Signals under -25 dB SNR considering bandwidth and 
%period vectors
...
%Results to evaluate the probability of false alarm
...
figure;
for i = 1:length(JNRVector)
    averageFPRAux = squeeze(averageFPR(i,:,:));

    [minval, minidx] = min(averageFPRAux(:));
    [j, k] = ind2sub(size(averageFPRAux), minidx);
    disp('---------------------Pfa-----------------------------------')
    disp('------------------------------------')
    disp(['-------------JNR: ' num2str(JNRVector(i)) ' dB-----------------------'])
    disp(['Pfa: ' num2str(minval) '+-' num2str(stdFPR(i,j,k))])
    disp('------------------------------------')
    disp(['Threshold: ' num2str(thresholdVector(j))])
    disp('------------------------------------')
    disp(['Window length: ' num2str(window_median_length_vector(k))])
