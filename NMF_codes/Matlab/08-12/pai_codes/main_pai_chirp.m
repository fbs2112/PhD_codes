...
% Run simulations
global bandwithLoop;
bandwithLoop = length(bandwidthVector);

GoFCanoDeteflag = zeros(length(PfaVector), MBlock,Segnumb*Loopnumb);
GoFBlockDeteflag = zeros(bandwithLoop, length(periodVector), length(JNRVector), length(PfaVector), MBlock,Segnumb*Loopnumb);

for bandwidthIndex = 1:bandwithLoop
