...
paramsNMF.alg = 'vanilla_ort_H';

mu_orth = [0 1e-2;0 1e-1;0 10;0 1e2];
...
xHat = zeros(totalSamples, numberOfSources, length(paramsNMF.JNRVector), length(numberOfZerosVector), length(bandwidthVector), monteCarloLoops, size(mu_orth, 1));

for muIndex = 1:size(mu_orth, 1)
    paramsNMF.mu_orth = mu_orth(muIndex,:);
            ...
            for numberOfZerosIndex = 1:length(numberOfZerosVector)
                ...
                for JNRIndex = 1:length(paramsNMF.JNRVector)
                    for i = 1:numberOfSources
                        ...
                        xHat(:,i,JNRIndex,numberOfZerosIndex,bandwidthIndex,loopIndex,muIndex) = xHatAux(edgeZeros+1:end-edgeZeros); %removing zeros at the edge
                    end
                end
            end
...
save(['.' filesep 'data' filesep 'nmf_testing_33.mat'], 'xHat', 'JNRVector', '-v7.3');
