...
%     formatFig(gcf, [dataPath  'gps_time'], 'en', figProp);
    ...
    [WSignal, HSignal, ~, PxxSignal, f, t] = nmf_eval_v2(mixtureGPS, params);
...
%     formatFig(gcf, [dataPath  'gps_spec'], 'en', figProp);
    ...
    Timeofthisloop = 0:totalSamples-1;
%     Carrphase = mod(2*pi*(paramsSignal.FreqDopp)*Timeofthisloop/paramsSignal.Freqsamp,2*pi);
    Carrphase = mod(2*pi*(2e3)*Timeofthisloop/paramsSignal.Freqsamp,2*pi);
    Carrier = exp(1i*Carrphase).';
    ...
    GPSSignals = GPSGen(paramsSignal);
    GPSSignals = GPSSignals(1:numberOfRawSamples,:);
%     GPSSignals = GPSSignals(numberOfRawSamples+1:numberOfRawSamples*2,:);
    GPSSignalsPower = pow_eval(GPSSignals);
    ...
    [~, H, errorH, Pxx, f, t] = nmf_eval_v2(mixtureSignal, params);
