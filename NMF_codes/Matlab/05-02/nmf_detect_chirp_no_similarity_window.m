...
expName = 'detect_chirp_no_similarity_window';
thresholdVector = 0.1:0.1:0.9;
% thresholdVector = 0.4;
monteCarloLoops = 50;

%Pre allocation------------------------------------------------------------
fp_window = zeros(monteCarloLoops, length(thresholdVector), length(params.JNRVector));
tp_window = zeros(monteCarloLoops, length(thresholdVector), length(params.JNRVector));
fn_window = zeros(monteCarloLoops, length(thresholdVector), length(params.JNRVector));
tn_window = zeros(monteCarloLoops, length(thresholdVector), length(params.JNRVector));
%--------------------------------------------------------------------------
...
for loopIndex = 1:monteCarloLoops
    ...
    for JNRIndex = 1:length(params.JNRVector)
        ...
        for thresholdIndex = 1:length(thresholdVector)
            ...
            %Variance window
            figure;
            HMean = window_eval(H{1, JNRIndex}(1,:), window_length, @var);
            ...
            if any(detection_res(1:onset))
                fp_window(loopIndex, thresholdIndex, JNRIndex) = fp_window(loopIndex, thresholdIndex, JNRIndex) + 1;
            else
                tn_window(loopIndex, thresholdIndex, JNRIndex) = tn_window(loopIndex, thresholdIndex, JNRIndex) + 1;
            end
            ...
            if any(detection_res(onset  + round(window_length/2):onset  + round(3*window_length/2)))...
                ...
                tp_window(loopIndex, thresholdIndex, JNRIndex) = tp_window(loopIndex, thresholdIndex, JNRIndex) + 1;
            else
                fn_window(loopIndex, thresholdIndex, JNRIndex) = fn_window(loopIndex, thresholdIndex, JNRIndex) + 1;
            end
...
save(['.' filesep 'data' filesep '04-10' filesep 'results03.mat'], 'fp_window', 'tp_window', 'tn_window', 'fn_window');
