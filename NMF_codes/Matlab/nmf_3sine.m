...
signal1 = [zeros(round(secondsOfSilence*fs), 1); signal1; zeros(round(secondsOfSilence*fs/2), 1)];
signal1Length = length(signal1);

signal2 = [signal2;zeros(signal1Length - signal2Length, 1)];
signal3 = sin(2*pi*f3*f.*t).';
signal3 = [zeros(round(secondsOfSilence*fs/4), 1); signal3];
signal3 = [signal3;zeros(signal1Length - length(signal3),1)];

signal1 = [signal1;zeros(round(secondsOfSilence*fs/5), 1)];
signal2 = [signal2;zeros(round(secondsOfSilence*fs/5), 1)];
signal3 = [signal3;zeros(round(secondsOfSilence*fs/5), 1)];

mixtureSignal = signal1 + signal2 + signal3;
...
for JNRIndex = 1:length(params.JNRVector)
    ...
    for dataIndex = 1:size(PxxAux, 1)
        ...
        if save_fig
            formatFig(gcf, [dataPath 'spectrogram_3sine'], 'en', figProp);
        end
        ...
        for i = 1:numberOfSources
            ...
            if save_fig
                formatFig(gcf, [dataPath 'basis_3sine_' num2str(i)], 'en', figProp);
            end
            figure;
            ...
            xlim([min(t) max(t)]*1e6);
            if save_fig
                formatFig(gcf, [dataPath 'activation_3sine_' num2str(i)], 'en', figProp);
            end
            %Mean window
            figure;
            HMean = window_eval(H{dataIndex, JNRIndex}(i,:), window_length, @mean);
            taux = linspace(0, max(t), length(HMean))*1e6;
            plot(taux*1e6,  HMean ./ max(HMean));
            ylabel('Normalized Magnitude');
            xlabel('Time [$\mu$s]');
            ylim([0 1.1])
            xlim([min(taux) max(taux)]*1e6);
            if save_fig
                formatFig(gcf, [dataPath 'activation_mean_3sine_' num2str(i)], 'en', figProp);
            end
        end
        ...
        xlim([1 idx])
        if save_fig
            formatFig(gcf, [dataPath 'loss_error_3sine'], 'en', figProp);
        end
        ...
        for k = 1:numberOfSources
            ...
            c.TickLabelInterpreter = 'latex';
            if save_fig
                formatFig(gcf, [dataPath 'output_spectrogram_3sine_' num2str(k)], 'en', figProp);
            end
        end
    end
end
