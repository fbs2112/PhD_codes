...
for loopIndex = 1:monteCarloLoops
    ...
    for JNRIndex = 1:length(params.JNRVector)
        ...
        for stdIndex = 1:length(stdVector)
            ...
            b = mle(output,'pdf',@(output,b)gampdf(output, params.nfft, b),'start',1e-3); % fix first parameter
            
            v = gamfit(output);
%             pd = fitdist(output, 'gamma');
            pd = makedist('gamma', 'a', params.nfft, 'b', b);
            [hgamma, pgamma] = chi2gof(output,'CDF', pd);
